-- Resolve staff office assignment when auth.uid() differs from admin_staff.id (email-linked rows).
-- Also used when Lambda still serves legacy staff_self_attendance_status without office_assigned.
...
CREATE OR REPLACE FUNCTION public._staff_attendance_employee_id()
RETURNS uuid
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.staff_office_assignments a WHERE a.employee_id = v_auth
  ) THEN
    RETURN v_auth;
  END IF;
  ...
  WHERE s.id = v_auth OR s.user_id = v_auth
  ORDER BY CASE WHEN s.id = v_auth THEN 0 ELSE 1 END
  LIMIT 1;

  IF v_staff_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM public.staff_office_assignments a WHERE a.employee_id = v_staff_id
  ) THEN
    RETURN v_staff_id;
  END IF;
  ...
  IF v_email IS NOT NULL THEN
    SELECT s.id INTO v_staff_id
    FROM public.admin_staff s
    WHERE lower(trim(s.email)) = lower(v_email)
    LIMIT 1;
...
BEGIN
  ...
  IF v_office.require_face THEN
    ...
    WHERE id = v_uid OR user_id = v_uid OR user_id = auth.uid() OR id = auth.uid()
    LIMIT 1;
...
GRANT EXECUTE ON FUNCTION public._staff_attendance_employee_id() TO authenticated;
