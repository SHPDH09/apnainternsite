-- RDS: site contact details + WhatsApp links (footer, registration, etc.)
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon')
    ...
    EXECUTE 'DROP POLICY IF EXISTS "Public read active site_contact_details" ON public.site_contact_details';
    EXECUTE $p$
    ...
    $p$;
    EXECUTE 'DROP POLICY IF EXISTS "Admins manage site_contact_details" ON public.site_contact_details';
    EXECUTE $p$
    ...
    $p$;
    EXECUTE 'DROP POLICY IF EXISTS "Public read active site_whatsapp_links" ON public.site_whatsapp_links';
    EXECUTE $p$
      CREATE POLICY "Public read active site_whatsapp_links"
        ...
        USING (is_active = true)
    $p$;
    EXECUTE 'DROP POLICY IF EXISTS "Admins manage site_whatsapp_links" ON public.site_whatsapp_links';
    EXECUTE $p$
    ...
    $p$;
    EXECUTE 'GRANT SELECT ON public.site_contact_details TO anon, authenticated';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON public.site_contact_details TO authenticated';
    EXECUTE 'GRANT SELECT ON public.site_whatsapp_links TO anon, authenticated';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON public.site_whatsapp_links TO authenticated';
  END IF;
EXCEPTION WHEN undefined_function OR undefined_object THEN
  RAISE NOTICE 'Skipping site_contacts RLS (auth roles/has_role not available)';
END $$;

INSERT INTO public.site_contact_details (contact_type, label, value, href, display_contexts, is_active, sort_order)
SELECT * FROM (VALUES
  ('address'::text, 'Office Address'::text, 'Arfabad Colony, East Nahar Road, Bajrangpuri, Patna - 800007, Bihar'::text, NULL::text, '["footer"]'::jsonb, true, 0),
  ('phone'::text, 'Phone'::text, '+91 70509 36593'::text, 'tel:+917050936593'::text, '["footer"]'::jsonb, true, 1),
  ('email'::text, 'Contact'::text, 'contact@ezyintern.in'::text, 'mailto:contact@ezyintern.in'::text, '["footer"]'::jsonb, true, 2),
  ('email'::text, 'Support'::text, 'support@ezyintern.in'::text, 'mailto:support@ezyintern.in'::text, '["footer"]'::jsonb, true, 3),
  ('social'::text, 'YouTube'::text, 'YouTube'::text, 'https://www.youtube.com/@Ezyintern_Internship'::text, '["footer"]'::jsonb, true, 10),
  ('social'::text, 'LinkedIn'::text, 'LinkedIn'::text, 'https://www.linkedin.com/company/ezyintern1/'::text, '["footer"]'::jsonb, true, 11)
