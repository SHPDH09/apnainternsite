...
async function ensurePartnerApplicationsTableCore(): Promise<void> {
  await query(`
...
export async function ensurePartnerApplicationsTables(): Promise<{ ok: true }> {
  ...
  await ensurePartnerApplicationsTableCore();

  try {
    await runPartnerApplicationsSqlFile();
  } catch (err) {
    const msg = String((err as { message?: string })?.message || err || "");
    console.warn("[partner-applications-bootstrap] optional coupon tables:", msg.slice(0, 220));
  }
